...
-->
<template>
    <div class="dragable_container">
        <div
            class="drag_left"
            ref="refLeftContanier"
            :style="{'width':`${leftWidth - leftMoveNum}px`,height:'100%',display:'inline-block','min-width':minWidthComp}"
        >
            <slot name="left"></slot>
        </div>
        <div
            class="drag_bar"
            @mousedown="handleSplitLine"
        >
            <i class="el-icon-d-caret"></i>
        </div>
        <div
            class="drag_right"
            :style="{width:`calc(100% - ${2 - leftMoveNum + leftWidth}px )`,height:'100%','min-width':minWidthComp}"
        >
